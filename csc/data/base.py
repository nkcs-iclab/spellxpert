...
class Template3(Template0):
    instruction = (
        f'指出句子中的错别字（可能有0、1或多个错别字），不需改正。'
        f'具体要求如下：\n'
        f'1. 如果发现错字，将文中错字逐个用{Template0.opening_tag}{Template0.closing_tag}标签包裹。'
        f'连续的错别字请分别使用标签包裹。例如AB为错字，应标记为{Template0.opening_tag}A{Template0.closing_tag}'
        f'{Template0.opening_tag}B{Template0.closing_tag}，而不是'
        f'{Template0.opening_tag}AB{Template0.closing_tag}。\n'
        f'2. 非错误字符照常输出。\n'
        f'3. 同一个词里如果只有一个字为错字，标记一个错的那个字即可，不要将正确的字包含进标记中。'
        f'例如“竟争”应该标记为{Template0.opening_tag}竟{Template0.closing_tag}争'
        f'而不是{Template0.opening_tag}竟争{Template0.closing_tag}。\n'
